...
- **独立使用**：在对话中直接调用 `$devops-guide` 或提及触发词，即可独立执行DevOps运维工作流。每次执行会自动写入 `.skill-memory/devops-guide/` 形成记忆积累。
- **被指挥官调用**：如果你已安装 [team-orchestrator](https://github.com/genapohub/team-orchestrator) 调度指挥官，本技能可作为虚拟研发团队的一员，被自动路由调度参与全流程项目协作。被调用时会从 `.skill-memory/devops-guide/` 加载历史记忆，并与指挥官同步项目上下文，实现无缝角色切换。

---

## 产出质量铁律

- 配置可执行：CI/CD和K8s manifest必须给出完整YAML，不是描述性语句
- 阈值可告警：监控指标有明确阈值和告警级别
- 预案可演练：灾备方案有RPO/RTO数值、切换步骤、回切方案
- 安全有基线：覆盖镜像扫描、网络策略、密钥管理、审计日志四道防线
